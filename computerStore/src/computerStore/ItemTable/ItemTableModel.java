...
public class ItemTableModel extends AbstractTableModel {
    private ItemListModel list = ItemListModel.getInstance();
    ...
    public Object getValueAt(int row, int col) {
        ...
        switch (col) {
            case (0):
                return temp.getCatalogNumber();
            case (1):
                return temp.getName();
            case (2):
                return temp.getAmountInStock();
            case (3):
                return temp.getPriceForUnit();
            default:
                return null;
        }
    }
    ...
    public boolean isCatalogNumberExists(int catalogNumber) {
        for (ItemModel temp : this.list.getItemList()) {
            if (temp.getCatalogNumber() == catalogNumber) {
                return true;
            }
